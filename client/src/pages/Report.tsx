    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-3">
        <div className="min-w-[130px] rounded-xl border border-gray-3 bg-white p-4">
          <p className="text-xs text-gray">{t('report.yearTotal')}</p>
        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-b border-gray-3 text-gray">
              ...
              <th className="px-3 py-2">{t('report.efficiencyShort')} %</th>
            </tr>
          </thead>
          <tbody>
            {monthly.map((m) => {
              ...
              return (
                <tr key={m.month} className="border-b border-gray-3 last:border-0">
                  <td className="px-3 py-2">{mlabel(m.month, language)}</td>
            ...
            {!loading && monthly.length === 0 && (
              <tr><td colSpan={5} className="px-3 py-6 text-center text-gray">{t('report.noDataHint')}</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <div>
        <p className="mb-2 text-sm font-medium text-navy">{t('report.byHouse')}</p>
          ...
          {byHouse.map((h) => {
            ...
            return (
              <div key={h.house_id} className="rounded-xl border border-gray-3 bg-white p-3 transition-shadow duration-150 hover:shadow-sm">
                ...
                <p className="text-xs text-gray">{h.months} {t('report.months')}</p>
