    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3 rounded-xl border border-gray-3 bg-white px-4 py-3 text-sm">
        ...
        <span className="flex items-center gap-1 text-gray">
          <span className={`h-2 w-2 rounded-full ${connected ? 'bg-brand-green' : connected === false ? 'bg-brand-red' : 'bg-gray-3'}`} />
      ...
      <div className="rounded-xl border border-gray-3 bg-white p-4">
        ...
        <div className="h-3 w-full overflow-hidden rounded-full bg-gray-3">
          <div className={`h-full rounded-full ${progressColor(efficiency)}`} style={{ width: `${Math.min(100, efficiency)}%` }} />
        </div>
      </div>
      ...
      <div>
        ...
        {!loading && houses.length === 0 && <p className="text-sm text-gray">வீடுகள் இல்லை.</p>}
      </div>

      <div className="rounded-xl border border-gray-3 bg-white p-4">
        <p className="mb-2 text-sm font-medium text-navy">நிலுவை உள்ள வீடுகள்</p>
        {dueHouses.length === 0 ? (
          <p className="text-sm text-gray">நிலுவை இல்லை. 🎉</p>
        ) : (
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {dueHouses.map((h) => {
                  const record = records[h.id];
                  return (
                    <tr key={h.id} className="border-t border-gray-3">
